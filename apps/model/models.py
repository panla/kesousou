...
class Conferences(AbstractModel):
    ...
    page = models.CharField('页码', max_length=10, null=True)
...
class Patent(AbstractModel):
    ...
    application_date = models.DateTimeField('专利申请日期', null=True)
    publication_number = models.CharField('专利公开号', max_length=50, unique=True, db_index=True)
    publication_date = models.DateTimeField('专利公告日期', null=True)
...
class Periodical(AbstractModel):
    ...
    doi = models.CharField('doi', max_length=100, unique=True, db_index=True)
    abstract = models.TextField('期刊简介', null=True)
    publish_date = models.DateField('发布日期', null=True)
    keywords = JSONField(verbose_name='关键词', null=True)
    creators = JSONField(verbose_name='作者')
    first_creator = models.CharField('第一作者', max_length=10, null=True)
    organizations = JSONField(verbose_name='作者机构')
    periodical_name = models.CharField('期刊名称', max_length=100, null=True)
    periodical_number = models.CharField('卷', max_length=10, null=True)
...
class Achievement(AbstractModel):
    ...
    publish_year = models.CharField('公布年份', max_length=20, null=True)
    category = models.CharField('中图分类', max_length=20, null=True)
    keywords = JSONField(verbose_name='关键词', null=True)
